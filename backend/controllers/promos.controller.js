import * as Model from "../models/promos.model.js";
...
export const get = async (req, res, next) => {
  try {
    ...
    if (!item) return res.status(404).json({ error: "Promo not found" });
    res.json(item);
  } catch (e) { next(e); }
};
...
export const getActive = async (req, res, next) => {
  try {
    if (Model.ensureTable) await Model.ensureTable();
    const item = await Model.findActive();
    if (!item) return res.status(404).json({ error: "No active promo" });
    res.json(item);
  } catch (e) { next(e); }
};
